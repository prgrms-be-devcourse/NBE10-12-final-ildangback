...
class ChallengeSeasonIntegrationTest extends IntegrationTestSupport {
    ...
    void recalculatesNextSeasonAndOwnerSettings() {
        ...
        // "오늘"만 고정한다. 서비스, Calculator, Repository는 실제 Spring 빈이다.
        ...
        Challenge next = challengeRepository.findByGroupIdAndSeqNo(groupId, 2).orElseThrow();
        ...
        assertThat(challengeRepository.findById(current.getId()).orElseThrow().getRequiredDayCount())
                .isEqualTo(5);
    }
}
