...
public class ChallengeController {
    ...
    @Operation(summary = "시즌 멤버 오늘 인증 현황 조회", description = "현재 챌린지에 참여 중인 멤버들의 오늘 인증 횟수를 조회합니다.")
    ...
    @Operation(summary = "READY 챌린지 설정 수정", description = "READY 상태의 챌린지 설정을 OWNER가 수정합니다.")
    ...
    @Operation(summary = "시즌 OWNER 위임", description = "현재 챌린지 OWNER가 다른 참여 멤버에게 OWNER 권한을 위임합니다.")
    ...
    public ResponseEntity<ExtensionChoiceResponse> updateExtensionChoice(
            @PathVariable Long challengeId,
            @CurrentUser SecurityUser actor,
            @Valid @RequestBody ExtensionChoiceRequest request) {
        ExtensionChoiceResponse response =
