...
import { getGroupHistoryDetail } from "../api";
import { formatDateTimeDot } from "../../../shared/lib/date";
import { PixelIcon } from "../../../shared/ui/PixelIcon";
import { pixelIcons } from "../../../shared/ui/pixelIcons";
import { TopBar } from "../../../shared/ui/TopBar";
import {
  GROUP_REASON_TYPE_LABEL,
  groupReasonSentence,
} from "../lib/groupReason";
import type { GroupPointHistoryResponse } from "../../../shared/api/types";

export function GroupPointHistoryDetailPage() {
  const { groupId, historyId } = useParams<{
    groupId: string;
    historyId: string;
  }>();
  const [history, setHistory] = useState<GroupPointHistoryResponse | null>(
    null,
  );
  const [error, setError] = useState(false);

  useEffect(() => {
    if (!groupId || !historyId) return;
    let cancelled = false;

    getGroupHistoryDetail(Number(groupId), Number(historyId))
      .then((detail) => {
...
function Detail({ history }: { history: GroupPointHistoryResponse }) {
    ...
    <>
      <div className="flex flex-col items-center pt-6 pb-9 text-center">
        ...
        <p className="mt-1.5 text-[14px] text-gray-500">
          {groupReasonSentence(history.reason, history.sourceName)}
        </p>
      </div>
      ...
      <dl className="overflow-hidden rounded-2xl border border-purple-200 [&>*+*]:border-t [&>*+*]:border-purple-100">
        ...
        <InfoRow
          label="내역 유형"
          value={GROUP_REASON_TYPE_LABEL[history.reason]}
        />
