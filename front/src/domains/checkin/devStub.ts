...
/**
 * dev 서버에서 실백엔드 없이 화면 작업하기 위한 스텁. checkin·challenge·group 엔드포인트는
 * 이미 다 있음(daily-logs 만 아직 미머지) — 걷어낼 땐 이 파일과 api.ts 의
 * `isCheckInStubEnabled()` 분기·import 를 지운다.
 */
...
const STUB_MY_CHALLENGES: MyChallengeSummary[] = [
  ...
  ...c,
  // 카드 커버는 3장/2장/4장 등으로 섞어서 빈 칸 채우기도 확인.
  recentMediaUrls: Array.from({ length: 4 - (c.challengeId % 3) }, (_, i) =>
    stubPhoto(`cover-${c.challengeId}-${i}`),
  ),
}));
