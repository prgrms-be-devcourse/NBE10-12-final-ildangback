...
/** 와이어프레임 2번 — 사진/영상 인증 안내 + 촬영 시작. */
export function CheckInIntro({ checkInType, onStart }: Props) {
  const isVideo = checkInType === "VIDEO";
  const label = isVideo ? "영상" : "사진";
  const Icon = isVideo ? VideoCameraIcon : CameraIcon;

  return (
    <div className="flex flex-1 flex-col items-center px-8 pt-6 pb-8 text-center">
      <CheckInHeader />

      <Icon size={72} weight="fill" className="mt-10 text-purple-400" />

      <h1 className="mt-6 text-[22px] font-bold text-gray-900">
        오늘의 인증을 <span className="text-purple-500">{label}</span>으로
        기록해요
      </h1>
      ...
      <ul className="mt-8 w-full space-y-4 text-left">
        <Hint icon={<CameraIcon size={20} />} title="즉석 촬영만 가능해요">
          앨범에서 {label}을 불러올 수 없어요
        </Hint>
        {isVideo ? (
          <Hint icon={<ClockIcon size={20} />} title="2초로 자동 종료돼요">
            녹화가 시작되면 정확히 2초 뒤 자동으로 멈춰요
          </Hint>
        ) : (
          <Hint
